function SE_SAM(Train_xNorm,Test_xNorm,Validate_xNorm,Train_yNorm,Test_yNorm,Validate_yNorm,yopt, ...
...
save_folder_output=[save_folder,'/se_sam'];
train=exist([save_folder_output,'/se_sam_model.mat']);
if train==0
...
tempLayers = [
    ...
    eluLayer(1,"Name","elu")
    maxPooling2dLayer(kernel_size2,'Padding','same','name','maxpool')
...
% 网络训练
% tic
se_sam_model = trainNetwork(Train_xNorm,Train_yNorm,lgraph,options);
save([save_folder_output,'/se_sam_model'],'se_sam_model');
else
    load ([save_folder_output,'/se_sam_model'],'se_sam_model');
end
%预测
YTrain_Pred =se_sam_model.predict(Train_xNorm);%YTrain_Pred=double(YTrain_Pred);
YTest_Pred = se_sam_model.predict(Test_xNorm);%YTest_Pred=double(YTest_Pred);
YValid_Pred = se_sam_model.predict(Validate_xNorm);%YValid_Pred=double(YValid_Pred);
%% 反归一化
%预测值反归一化
