function DCBAM(Train_xNorm,Test_xNorm,Validate_xNorm,Train_yNorm,Test_yNorm,Validate_yNorm,yopt, ...
...
save_folder_output=[save_folder,'/dcbam'];
train=exist([save_folder_output,'/dcbam_model.mat']);
if train==0
...
for i=1:num_block
   ...
   % 通道注意力机制
    tempLayers = [
        globalAveragePooling2dLayer("Name","c_gapool_"+i)
        fullyConnectedLayer(channel*2,"Name","a_fc_1"+i)
        eluLayer(1)
        fullyConnectedLayer(channel*2,"Name","a_fc_2"+i)
        sigmoidLayer("Name","a_sigmoid_1"+i)];
    lgraph = addLayers(lgraph,tempLayers);
    
    tempLayers = [
        globalMaxPooling2dLayer("Name","c_gmpool_"+i)
        fullyConnectedLayer(channel*2,"Name","m_fc_1"+i)
        eluLayer(1)
        fullyConnectedLayer(channel*2,"Name","m_fc_2"+i)
        sigmoidLayer("Name","a_sigmoid_2"+i)];
    lgraph = addLayers(lgraph,tempLayers);
    tempLayers = additionLayer(2,"Name","addition_"+i);
    lgraph = addLayers(lgraph,tempLayers);
    % 空间注意力机制
  ...
  % 添加通道注意力机制
    lgraph = connectLayers(lgraph,"concat"+i,"c_gapool_"+i);
    lgraph = connectLayers(lgraph,"concat"+i,"c_gmpool_"+i);
    lgraph = connectLayers(lgraph,"a_sigmoid_1"+i,"addition_"+i+"/in1");
    lgraph = connectLayers(lgraph,"a_sigmoid_2"+i,"addition_"+i+"/in2");
    % 添加空间注意力机制
    ...
    % 结果相乘
    lgraph = connectLayers(lgraph,"addition_"+i,"multiplication"+i+"/in1");
...
% 网络训练
% tic
dcbam_model = trainNetwork(Train_xNorm,Train_yNorm,lgraph,options);
save([save_folder_output,'/dcbam_model'],'dcbam_model');
else
    load ([save_folder_output,'/dcbam_model'],'dcbam_model')
end
%预测
YTrain_Pred =dcbam_model.predict(Train_xNorm);%YTrain_Pred=double(YTrain_Pred);
YTest_Pred = dcbam_model.predict(Test_xNorm);%YTest_Pred=double(YTest_Pred);
YValid_Pred = dcbam_model.predict(Validate_xNorm);%YValid_Pred=double(YValid_Pred);
%% 反归一化
%预测值反归一化
