function DSelfAttention(Train_xNorm,Test_xNorm,Validate_xNorm,Train_yNorm,Test_yNorm,Validate_yNorm,yopt, ...
...
numHiddenUnits=100;
save_folder_output=[save_folder,'/dselfattention'];
train=exist([save_folder_output,'/dselfattention_model.mat']);
if train==0
...
% 网络训练
% tic
dselfattention_model = trainNetwork(Train_xNorm,Train_yNorm,lgraph,options);
save([save_folder_output,'/dselfattention_model'],'dselfattention_model');
else
    load ([save_folder_output,'/dselfattention_model'],'dselfattention_model')
end
%预测
YTrain_Pred =dselfattention_model.predict(Train_xNorm);%YTrain_Pred=double(YTrain_Pred);
YTest_Pred = dselfattention_model.predict(Test_xNorm);%YTest_Pred=double(YTest_Pred);
YValid_Pred = dselfattention_model.predict(Validate_xNorm);%YValid_Pred=double(YValid_Pred);
%% 反归一化
%预测值反归一化
